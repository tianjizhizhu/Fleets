  ...
  <div class="page-content py-8">
    <div class="card p-5 mb-6">
      <div class="flex items-center justify-between mb-3">
        <h3 class="font-medium text-ink font-display">标注进度</h3>
      ...
      <div class="flex justify-between mt-2 text-xs text-ink-faint">
        <span>{{ progressPercentage }}% 完成</span>
        <span>已标注 {{ formatDuration(totalDuration) }}</span>
      </div>
    </div>

    <div v-if="isBatchMode && selectedRecords.size > 0" class="card p-5 mb-6 bg-accent/5 border-accent/20">
      <div class="flex items-center justify-between mb-3">
      ...
      <p class="text-sm text-ink-muted mb-3">批量标注到：</p>
      <div class="flex flex-wrap gap-2">
        <button
          v-for="category in userStore.categories"
          :key="category.id"
          @click="batchAnnotate(category.id)"
          class="px-4 py-2 bg-white border border-paper-3 rounded-xl text-sm hover:border-accent hover:text-accent transition-all"
        >
          {{ category.name }}
        </button>
      </div>
    ...
    <div v-if="pendingRecords.length === 0" class="empty-state">
      ...
      <p class="text-ink-muted mb-2 font-display">太棒了！</p>
      <p class="text-sm text-ink-faint">所有记录都已标注完成</p>
    </div>
    ...
    <div v-else class="space-y-3">
      <div
        ...
        :class="[
          ...
          selectedRecords.has(record.id) && 'ring-2 ring-accent bg-accent/5'
        ]"
      >
        <div class="flex items-start gap-3">
          <button
            ...
            :class="[
              'w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 mt-1 transition-all',
              selectedRecords.has(record.id)
                ? 'bg-accent border-accent'
                : 'border-paper-3 hover:border-accent'
            ]"
          >
            <svg
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                ...
                class="px-3 py-1.5 text-sm bg-paper-2 text-ink-muted rounded-xl hover:bg-accent hover:text-white transition-all"
              >
                {{ category.name }}
              </button>
            </div>
